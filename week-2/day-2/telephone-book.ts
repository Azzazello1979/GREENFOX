...
/* function searchInPhoneBook(map:{[i:string]:string}, name?:any, number?:string){
  if(number===undefined){
    phoneBook.hasOwnProperty(name) ? 
    console.log(`${name}: ${phoneBook[name]}`) : 
    console.log(`We dont know ${name}'s phone number`);

  }else Object.keys(phoneBook).forEach(element => 
    phoneBook[element] === '307-687-2982' ? 
    console.log(`Number ${phoneBook[element]} belongs to ${element}`) : 
    undefined
    );
} */
